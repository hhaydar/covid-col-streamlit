...
st.map(df.query("price>=800", engine='python')[["latitude", "longitude"]].dropna(how="any"))
st.subheader("In a table")
st.markdown("Following are the top five most expensive properties.")
st.write(df.query("price>=800", engine='python').sort_values("price", ascending=False).head())
...
top_host_1 = df.query('host_id==@listingcounts.index[0]', engine='python')
top_host_2 = df.query('host_id==@listingcounts.index[1]', engine='python')
...
f = px.histogram(df.query(f"price.between{values}", engine='python'), x="price", nbins=15, title="Price distribution")
...
def get_availability(show_exp, neighborhood):
    return df.query(f"""neighbourhood_group==@neighborhood{show_exp}\
        and availability_365>0""", engine='python').availability_365.describe(\
...
if minimum > maximum:
    ...
    df.query("@minimum<=number_of_reviews<=@maximum", engine='python').sort_values("number_of_reviews", ascending=False)\
